...
export function ModeToggle({ mode, onChange }: Props) {
  return (
    <div className="space-y-2">
      <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">What do you want to calculate?</p>
      <div className="grid grid-cols-3 gap-1.5 bg-gray-100 p-1 rounded-xl">
        {MODES.map(({ value, label, desc }) => (
          <button
            ...
            title={desc}
            className={`
              py-2.5 px-1 rounded-lg text-sm font-semibold transition-all duration-150 min-h-[48px]
